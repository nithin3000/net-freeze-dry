 ...
 *
 * 2014 Purdue University, All Rights Reserved
 */
...
#define CHANNEL_0_CHAN_ID_DEV_NUM       ((uint8_t) 0x01)      ///< Device number
 ...
 static void  pcap_spi_set(spi_master_hw_instance_t mod_num)
{
	...
	#if defined(SPI_MASTER_0_ENABLE)
        spi_config.SPI_Pin_SCK = SPI_PSELSCK0;
        spi_config.SPI_Pin_MISO = SPI_PSELMOSI0;
        spi_config.SPI_Pin_MOSI = SPI_PSELMISO0;
        spi_config.SPI_Pin_SS = SPI_PSELSS0;
    #elif defined(SPI_MASTER_1_ENABLE)
        spi_config.SPI_Pin_SCK = SPI_PSELSCK1;
        spi_config.SPI_Pin_MISO = SPI_PSELMISO1;
        spi_config.SPI_Pin_MOSI = SPI_PSELMOSI1;
        spi_config.SPI_Pin_SS = SPI_PSELSS1;
    #endif /* SPI_MASTER_ENABLE */
  uint32_t err_code = spi_master_open(mod_num, &spi_config);
	APP_ERROR_CHECK(err_code);
}
/***** Transmits/Recieves data using SPI. ****
 * @param PCAP_spi_address Pointer c	ontaining the set spi address 
 ...
 * @retval false Error occurred
 */
bool pcap_spi_tx_rx(uint8_t MSG_LEN, uint8_t * const tx_data)
{
	uint32_t return_value;
	
	return_value =	spi_master_send_recv(SPI_MASTER_HW, tx_data, MSG_LEN, rx_data, MSG_LEN);
		...
		for (x = 0; x < 4096; x++)
			{
				...
				b = pcap_spi_tx_rx( MSG_LEN, tx_data); // test submission
				memset(tx_data, 0, 8);
				memset(rx_data, 0, 8);		
			}
			return b;
...
bool pcap_config_write(uint32_t *regdata) // why does the pointer work here
	{
...
uint32_t read_reg(uint8_t wr_reg_add) 
	{ 
		...
		//nrf_delay_ms(DELAY_MS);
		meas = pcap_spi_tx_rx( MSG_LEN, tx_data); //Intiate the read of capacitance data
...
//	}
	...
	while(true)
	{
		...
		sw = nrf_gpio_pin_read(BUTTON1); // 0 indicates button pressed & 1 indicates button releas
		...
		while(sw == 0)
		{
		  ...
		  nrf_delay_ms(100);
			...
			ret1 = pcap_spi_tx_rx( MSG_LEN, tx_data);
			nrf_delay_ms(100);//use to be 100
			...
			nrf_delay_ms(100);// used to be 100
			if (ret2 == 0)
			{
			...
			*/
			...
			memset(tx_data, 0, 8);
			memset(rx_data, 0, 8);
			cap_t[0] = read_reg(read_reg0);
			
			/* Measured Capacitors */
			memset(tx_data, 0, 8);
			memset(rx_data, 0, 8);
			cap_t[1] = read_reg(read_reg1);
			
			memset(tx_data, 0, 8);
			memset(rx_data, 0, 8);
			cap_t[2] = read_reg(read_reg2);
			
			memset(tx_data, 0, 8);
			memset(rx_data, 0, 8);
			cap_t[3] = read_reg(read_reg3);
			
			memset(tx_data, 0, 8);
			memset(rx_data, 0, 8);
			cap_t[4] = read_reg(read_reg4);
			
			memset(tx_data, 0, 8);
			memset(rx_data, 0, 8);
			cap_t[5] = read_reg(read_reg5);
			
			memset(tx_data, 0, 8);
			memset(rx_data, 0, 8);
			cap_t[6] = read_reg(read_reg6);
			
			memset(tx_data, 0, 8);
			memset(rx_data, 0, 8);
			cap_t[7] = read_reg(read_reg7);
